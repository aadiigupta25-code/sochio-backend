...
io.on("connect", async (socket) => {
  ...
  socket.on("disconnect", async (data) => {
    ...
    if (seller) {
      ...
      if (liveSeller) {
        await liveSeller.deleteOne();
        console.log("liveSeller deleted in disconnect");
      ...
      sockets?.length ? io.socketsLeave("liveSellerRoom:" + seller.liveSellingHistoryId?.toString()) : console.log("sockets not able to leave in disconnect");
    }
  });
});
